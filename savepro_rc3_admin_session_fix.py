# SavePro RC3 — admin personal-session priority fix
...
async def build_personal_client(user_id: int):
    # Prefer the persistent SQLite phone session because it also keeps
    # Pyrogram's peer/access-hash cache for private t.me/c/... chats.
...
async def persist_phone_login(user_id: int, temp_client, temp_name: str):
    """Keep Phone+OTP login as a persistent Pyrogram SQLite session.

    This preserves the peer/access-hash cache needed for private t.me/c links.
    Session String login remains available separately as the advanced method.
    """
    me = await asyncio.wait_for(temp_client.get_me(), timeout=LOGIN_STEP_TIMEOUT)

    try:
        await asyncio.wait_for(
            initialize_peer_cache(temp_client, f"user:{user_id}", force=True),
    ...
    stable_base = phone_session_path(user_id)
    string_path = session_string_path(user_id)
    for path in (
        stable_base,
        stable_base + "-journal",
        stable_base + "-wal",
    ...
    temp_base = os.path.join(SESSION_DIR, f"{temp_name}.session")
    if not os.path.exists(temp_base):
        raise RuntimeError("Telegram login session file was not created.")

    os.replace(temp_base, stable_base)
    for suffix in ("-journal", "-wal", "-shm"):
        src_aux = temp_base + suffix
        dst_aux = stable_base + suffix
        if os.path.exists(src_aux):
            os.replace(src_aux, dst_aux)

    try:
        os.chmod(stable_base, 0o600)
    except OSError:
        pass
    ...
    PEER_CACHE_INITIALIZED.add(f"user:{user_id}")
    PEER_RESYNC_AT[f"user:{user_id}"] = 0.0
    return me
...
async def get_user_client(user_id: int):
    ...
    async with get_user_lock(user_id):
        ...
        personal_exists = has_personal_session(user_id)
        if personal_exists:
            try:
            ...
            # Important: do not silently switch an admin to STRING_SESSION.
            # Otherwise /loginstatus can look correct while private t.me/c links
            # are actually being fetched by a different Telegram account.
            return None

        # Shared server session is only a fallback when NO personal login exists.
...
async def fetch_message_safely(ub_client, cache_key, chat_id, msg_id, status_msg=None):
    try:
        ...
        if status_msg:
            await _safe_status_edit(status_msg, f"🔄 Reading ID {msg_id} • retrying once…")
        ...
        if status_msg:
            await _safe_status_edit(status_msg, f"🔄 Refreshing source access • ID {msg_id}…")
        ...
        try:
            ...
            PEER_RESYNC_AT[cache_key] = time.monotonic()
...
async def login_status_cmd(client, message: Message):
    ...
    if personal:
        ub = await get_user_client(user_id)
        if ub and ub is not server_userbot:
            try:
                me = await ub.get_me()
                ...
                pass
        return await message.reply_text(
            "⚠️ Personal login file exists but could not be opened. "
            "Use Logout, then Phone Login again."
        )
