...
# Legacy setting accepted for compatibility; completed episode receipts are always retained.
KEEP_EPISODE_PROGRESS = os.environ.get("KEEP_EPISODE_PROGRESS", "true").lower() in {
    "1", "true", "yes", "on"
}
...
def manual_text(user_id: int):
    return _legacy_manual_text(user_id) + tr(user_id,
        "\n\nNew flow: request → review summary → Start. Each episode shows Read/Download and Send/Upload progress, then a receipt after delivery. Final summary comes last. Pause/Resume/Cancel buttons are on progress cards. Retry failed only excludes uncertain deliveries. Home → Quick Branding Setup stages name and thumbnail until Apply.",
        "\n\nபுதிய flow: request → summary → Start. ஒவ்வொரு episode-க்கும் progress மற்றும் delivery receipt வரும்; கடைசியில் final summary. Progress card-ல் Pause/Resume/Cancel buttons. Retry failed only உறுதியான failures மட்டும். Home → Quick Branding Setup-ல் Apply செய்தால்தான் settings சேமிக்கப்படும்.") + tr(user_id,
...
async def request_job_confirmation(message, uid, kind, source, topic, ids, destination):
    ...
    while True:
        ...
        body = (f"📋 Ready to start • {kind.title()}\n"
                ...
                f"Audio title: {prefs.get('audio_title') or 'Original'}\n"
                f"Audio layout: {'Clean card' if prefs.get('audio_clean_card') else 'With caption'}\n"
...
async def run_transfer_job(client, message, uid, kind, start_link=None, end_link=None, count=None, parent=None):
    ...
    try:
        ...
        for index, mid in enumerate(ids, 1):
            ...
            active_card = await message.reply_text(_batch_episode_progress_body(ctx), reply_markup=job_buttons(uid))
            try:
                ...
                if state == 'sent':
                    ctx['delivered'] = True
                # Final episode receipt follows the actual uploaded content.
                try:
                    await message.reply_text(_batch_episode_progress_body(ctx), parse_mode=ParseMode.DISABLED)
                    await safe_delete_message(active_card)
                except Exception:
                    pass
                active_card = None
        ...
        await _safe_status_edit(overall, body)
        rows = []
        if failed:
...
async def start_cmd(client, message: Message):
    ...
    keyboard_rows = [
        [InlineKeyboardButton("📥 Single Save", callback_data="save_single"),
         InlineKeyboardButton("📦 Batch", callback_data="batch_new"),
         InlineKeyboardButton("♻ Clone", callback_data="save_clone")],
        [InlineKeyboardButton("🎧 Quick Branding Setup",callback_data="brand:start")],
        [
