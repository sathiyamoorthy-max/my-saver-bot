# SavePro FINAL v4 — moving progress card + anti-reading-stall
...
async def fetch_message_safely(ub_client, cache_key, chat_id, msg_id, status_msg=None):
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"Telegram did not respond for message {msg_id}; item skipped."
            ) from second_timeout
...
def _batch_episode_progress_body(ctx, phase=None, current=0, total_bytes=0, speed=0.0):
    ...
    lines = [
        f"🎧 Episode {index}/{total_items} • ID {msg_id}",
        f"📄 {file_name}",
        "",
        f"{active_phase}",
    ]
    ...
    lines.extend([
        "",
        f"Overall: {ctx.get('completed_before', 0)}/{total_items} completed",
    ])
    return "\n".join(lines)
