...
import os
BOT_VERSION = "2026.09.25-login-fix3"
import re
import asyncio
import time
import sqlite3
...
async def fetch_message_safely(ub_client, cache_key, chat_id, msg_id, status_msg=None):
    try:
        ...
        # Network failures are not peer-cache failures. Do not scan all chats.
        if type(error).__name__ != 'PeerIdInvalid':
            raise
        if status_msg:
            await _safe_status_edit(status_msg, "🔄 Syncing accessible Telegram chats…")
        await initialize_peer_cache(ub_client, cache_key)
        return await retry_flood(ub_client.get_messages, chat_id, msg_id)
