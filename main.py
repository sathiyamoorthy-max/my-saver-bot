...
# Admin: 50000 by default. Set ADMIN_MAX_BATCH_MESSAGES=0 for no per-job cap.
ADMIN_MAX_BATCH_MESSAGES = max(0, int(os.environ.get("ADMIN_MAX_BATCH_MESSAGES", "50000")))
...
def _render_user_template(template: str, target_msg: Message, original: str = ""):
    ...
    replacements = {
        ...
        "{filename}": _media_display_name(target_msg),
...
def _effective_caption(user_id: int, target_msg: Message):
    ...
    # Backward-compatible server-wide caption remains as a fallback.
    caption = CUSTOM_CAPTION if CUSTOM_CAPTION else original
    changed = bool(CUSTOM_CAPTION)
    ...
    if customization_allowed(user_id, "caption"):
        ...
        if mode == "replace":
            ...
            caption = ""
            changed = True
        elif mode == "original" and not CUSTOM_CAPTION:
            caption = original
            changed = False
    ...
    if (
        ...
        caption = "\n\n".join(x for x in (caption, watermark) if x)
        changed = True

    # Telegram media caption safe headroom.
    return (caption or "")[:1000], changed
...
def _effective_text(user_id: int, target_msg: Message):
    ...
    if (
        ...
        body = "\n\n".join(x for x in (body, watermark) if x)
        changed = True

    return (body or "")[:4000], changed
...
def customization_summary(user_id: int):
    ...
    return (
        ...
        "💡 Fast watermark is added to caption/text, so media quality is not re-encoded."
    )
...
def customization_preview_text(user_id: int):
    prefs = get_user_customization(user_id)
    original = "Original caption / text example"
    fake_name = "episode_001.mp4"

    caption_text = prefs.get("caption_text") or "(not set)"
    watermark = prefs.get("watermark_text") or "(not set)"
    prefix = prefs.get("text_prefix") or "(not set)"
    suffix = prefs.get("text_suffix") or "(not set)"

    return (
        "👁 **CUSTOMIZATION PREVIEW / STATUS**\n\n"
        f"✍️ Caption mode: `{prefs.get('caption_mode', 'original')}`\n"
        f"Caption text: `{caption_text[:180]}`\n\n"
        f"🏷 Watermark: `{'ON' if int(prefs.get('watermark_enabled') or 0) else 'OFF'}`\n"
        f"Watermark text: `{watermark[:180]}`\n\n"
        f"🖼 Thumbnail: `{'Custom thumbnail set' if _effective_thumbnail(user_id) else 'Default/original'}`\n\n"
        f"📝 Keep original text: `{'YES' if int(prefs.get('keep_original_text') or 0) else 'NO'}`\n"
        f"Prefix: `{prefix[:140]}`\n"
        f"Suffix: `{suffix[:140]}`\n\n"
        "Placeholders: `{filename}` • `{id}` • `{date}` • `{original}`"
    )
...
def _batch_overall_body(ctx, final=False, cancelled=False):
    ...
    return (
        ...
        + ("\n\n🎉 All requested items were processed." if final else "")
        + ("\n\nStop: `/cancel`" if not final and not cancelled else "")
    )
...
async def callback_handler(client, query: CallbackQuery):
    ...
    if query.data == "help_batch":
        await query.message.reply_text(
            ...
            "`/batch <start_link> -50`\n\n"
            f"Your per-batch maximum = `{batch_limit_text(user_id)}`.\n"
            "Admin default = `50,000` (set ADMIN_MAX_BATCH_MESSAGES=0 for unlimited)."
        )
        return
...
async def customization_text_input_handler(client, message: Message):
    ...
    if step == "caption_text":
        ...
        set_user_customization(user_id, caption_text=value, caption_mode="replace")
        CUSTOMIZE_STATES.pop(user_id, None)
        await message.reply_text(
            "✅ Custom caption saved. Mode changed to **Replace**.\n"
            "You can switch to Append/Original anytime from Settings.",
            reply_markup=settings_keyboard(user_id),
        )
        return
...
async def cancel_cmd(client, message: Message):
    ...
    if user_id in CUSTOMIZE_STATES:
        CUSTOMIZE_STATES.pop(user_id, None)
        await message.reply_text(
        ...
        for i in range(start_msg_id, latest_msg_id + 1):
            ...
            try:
                ...
                if has_downloadable_media(target_msg):
                    ...
                    await _update_clone_progress(msg, progress_ctx)
                    sent_ok = await send_non_file_content(client, dest_chat, target_msg, user_id=user_id)
                    if sent_ok:
                        success_count += 1
                        progress_ctx["success"] = success_count
                        progress_ctx["phase"] = "✅ Content sent • preparing next item…"
...
async def main():
    ...
    await bot.set_bot_commands([
        ...
        BotCommand("settings", "⚙️ Caption / Watermark / Thumbnail"),
