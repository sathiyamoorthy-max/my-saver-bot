...
def feature_allowed(user_id: int, feature: str, requested: int = 1):
    ...
    if left is not None and requested > left:
        return False, tr(
            user_id,
            f"⚠️ Today's saves are finished or insufficient. Remaining: {left}. Resets at 00:00 UTC (05:30 IST). /trial for details or /plans to upgrade.",
            f"⚠️ இன்றைய saves போதவில்லை. மீதம்: {left}. காலை 05:30 IST-க்கு புதுப்பிக்கப்படும். /trial அல்லது /plans பார்க்கவும்.",
        )
    return True, ""
...
def quick_start_text(user_id: int):
    ...
    return tr(user_id,
        ...
        f"2. Paste one Telegram post link to save it.\n"
        f"3. Free plan: {free_limit} Single saves per day.\n"
        "4. Multiple posts: /batch. Full source: /clone <post link>.\n"
        "5. Review plans with /plans and branding with /settings.\n\n"
        "Use sources your connected account can access.",
        f"⚡ QUICK START\n\n1. /login மூலம் Telegram account இணைக்கவும்.\n"
        "2. ஒரு Telegram post link அனுப்பவும்.\n"
        f"3. Free plan: தினமும் {free_limit} Single saves.\n"
        "4. பல posts: /batch. முழு source: /clone <post link>.\n"
        "5. Plans: /plans. Branding: /settings.\n\n"
        "உங்கள் account access உள்ள source பயன்படுத்தவும்.")
...
async def safe_delete_message(message: Message):
    try:
        await bounded_operation(message.delete(), "Removing temporary message", 5)
    except Exception:
        pass
...
def extract_telegram_links(text: str):
    links = re.findall(r"(?<![\w./])(?:https?://)?t\.me/[^\s<>]+", text or "", flags=re.I)
    return [link.rstrip('.,;)') if link.lower().startswith(('http://','https://'))
            else 'https://' + link.rstrip('.,;)') for link in links]
...
# ============================================================
# Deadline enforcement also bounds cancellation cleanup. An uncooperative SDK
# operation quarantines new jobs until it really exits; never retry over it.
JOB_CONTEXT = ContextVar("savepro_job", default=None)
RECOVERING_OPERATIONS = set()
RPC_TIMEOUT = 60
CANCEL_GRACE = 5

async def stop_operation(task):
    if task.done():
        return
    task.cancel()
    ...
    if done and not task.cancelled():
        task.exception()
    if not done:
        RECOVERING_OPERATIONS.add(task)
        def drained(finished):
            RECOVERING_OPERATIONS.discard(finished)
            if not finished.cancelled():
                finished.exception()
        task.add_done_callback(drained)
        raise asyncio.CancelledError("Network operation still stopping")

async def bounded_operation(coro, label, timeout=None):
    uid = JOB_CONTEXT.get()
    ...
    try:
        done, _ = await asyncio.wait({task}, timeout=RPC_TIMEOUT if timeout is None else timeout)
        if task in done:
            return await task
        raise asyncio.TimeoutError(label)
    finally:
        try:
            await stop_operation(task)
        finally:
            meta['phase'] = old or 'Processing'

async def latest_source_id(ub, source, first):
    async def read():
        async for latest in ub.get_chat_history(source, limit=1):
            return latest.id
        return first
    return await bounded_operation(read(), 'Checking latest source message')
...
async def build_personal_client(user_id: int):
    ...
    if saved_string:
        ...
        await bounded_operation(client.start(), "Connecting Telegram account")
        return client
...
async def initialize_peer_cache(ub_client, cache_key, force=False):
    if cache_key in PEER_CACHE_INITIALIZED and not force:
        return
    async def sync():
        async for _ in ub_client.get_dialogs(limit=500):
            pass
...
def is_peer_lookup_error(error):
    return (type(error).__name__ == 'PeerIdInvalid' or
            isinstance(error, ValueError) and str(error).lower().startswith('peer id invalid'))

async def recover_source_peer(ub_client, chat_id):
    # Resolve through the connected account's own dialogs; no guessed access hashes.
    # Targeted scanning stops as soon as Telegram supplies the requested chat.
    async def sync_source():
        if isinstance(chat_id, str):
            await ub_client.get_chat(chat_id)
            return
        async for dialog in ub_client.get_dialogs(limit=0):
            if getattr(getattr(dialog, 'chat', None), 'id', None) == chat_id:
                return
    await bounded_operation(sync_source(), 'Finding source in connected account chats')
...
async def fetch_message_safely(ub_client, cache_key, chat_id, msg_id, status_msg=None):
    try:
        ...
        if not is_peer_lookup_error(error):
            raise
        if status_msg:
            await _safe_status_edit(status_msg, "🔄 Finding source in your connected account…")
        # Repeat per actual lookup failure, not a stale user-id cache flag after login.
        await recover_source_peer(ub_client, chat_id)
        return await retry_flood(ub_client.get_messages, chat_id, msg_id)
...
def _batch_episode_progress_body(ctx, phase=None, current=0, total_bytes=0, speed=0.0):
    lines = [f"🎧 EPISODE {ctx.get('index',1)}/{ctx.get('total',1)} • ID {ctx.get('msg_id','?')}",
             str(ctx.get('file_name') or '')[:80]]
    labels = [('download','📄 Read'),('upload','📨 Send')] if ctx.get('non_file') else [('download','📥 Download'),('upload','📤 Upload')]
    for prefix,label in labels:
        amount=ctx.get(prefix+'_current',0)
        total=ctx.get(prefix+'_total',0)
        pct=100 if ctx.get('delivered') else min(100,100*amount/total) if total else 0
        lines.append(f"{label} {_progress_bar(pct)} {pct:.0f}%")
        if total:
            lines.append(f"{_human_bytes(amount)} / {_human_bytes(total)}")
    lines.append(phase or ctx.get('phase','Preparing…'))
    if speed:
        lines.append(f"Speed {_human_bytes(speed)}/s")
    lines.append(f"Overall processed: {ctx.get('completed_before',0)}/{ctx.get('total',1)}")
    return '\n'.join(lines)
...
def make_transfer_progress(status_msg, phase: str, msg_id: int, heartbeat=None, progress_ctx=None):
    ...
    async def progress(current, total):
        ...
        meta = JOB_META.get(JOB_CONTEXT.get())
        if meta is not None:
            meta.update(phase=phase, transfer_bytes=current, transfer_total=total,
                        last_progress=heartbeat['last_progress'])
...
async def _await_transfer_with_watchdog(coro, heartbeat, hard_timeout, label, msg_id):
    ...
    meta = JOB_META.get(JOB_CONTEXT.get(), {})
    meta['phase'] = f'{label} • message {msg_id}'
    try:
        while True:
...
async def transfer_media_original(ub, bot_client, dest_chat, target_msg, status_msg=None, progress_ctx=None, user_id=None):
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            pending = set(RECOVERING_OPERATIONS)
            def clean_transfer(_=None, pending=pending, directory=transfer_dir):
                if all(task.done() for task in pending):
                    shutil.rmtree(directory, ignore_errors=True)
            if pending:
                for operation in pending:
                    operation.add_done_callback(clean_transfer)
            clean_transfer()
    ...
    def decorate(fn):
        ...
        async def launch(client, message, *args, **kwargs):
            ...
            if RECOVERING_OPERATIONS:
                return await message.reply_text("⏳ A network operation is still stopping. New jobs are temporarily blocked to prevent duplicate sends. Try /status shortly; contact /support if this persists.")
            if uid in JOB_TASKS:
            ...
            async def runner():
                ...
                try:
                    ...
                    outcome = "cancelled"
                    try:
                        notice = ("⏳ Job interrupted. A network operation is still stopping; check delivery before retrying. /status" if RECOVERING_OPERATIONS else tr(uid, "🛑 Job stopped. /history shows recorded results.", "🛑 வேலை நிறுத்தப்பட்டது. முடிவுகளை /history மூலம் பார்க்கலாம்."))
                        await bounded_operation(message.reply_text(notice), "Sending stop confirmation", 10)
                    except Exception:
                        pass
...
CONFIRM_JOBS = os.environ.get("CONFIRM_JOBS", "true").lower() in {"true", "1", "yes"}
BRAND_STATES = {}
CLONE_INPUTS = set()
...
def friendly_error(error, uid):
    ...
    if isinstance(error, (asyncio.TimeoutError, ConnectionError)):
        return tr(uid, "Connection timed out. Check delivery before retrying.", "Connection timeout. Retry முன் file வந்ததா என்று பார்க்கவும்.")
...
def job_buttons(uid):
    ...
    return InlineKeyboardMarkup([[
        ...
        InlineKeyboardButton("▶ Resume", callback_data=f"job:resume:{jid}"),
...
def overall_card(kind, total, processed, sent, skipped, failed, uncertain, ending=None):
    ...
    title = ending or f"{kind.title()} • In progress"
    return (f"📦 {title}\n{_progress_bar(pct)} {pct:.0f}% processed\n"
            f"Processed {processed}/{total} • Sent {sent}\nSkipped {skipped} • Failed {failed} • Check delivery {uncertain}")

async def request_job_confirmation(message, uid, kind, source, topic, ids, destination):
    # Single Save should feel instant. Batch/Clone keep a safety review step.
    ...
    try:
        if parent:
            ...
            if kind == 'clone':
                ...
                last = await latest_source_id(ub, source, first)
                if last < first:
        ...
        for index, mid in enumerate(ids, 1):
            ...
            if parent:
                with db_conn() as con:
                    pending = con.execute("SELECT 1 FROM job_items WHERE job_id=? AND message_id=? AND status='failed'",(parent,mid)).fetchone()
                if not pending:
                    continue
            state, error = 'failed', ''
            ctx = {'kind':'batch_episode','index':index,'total':total,'completed_before':processed,
                   'msg_id':mid,'file_name':f'Message {mid}','ui_seq':0,'phase':'🔎 Reading message',
                   'download_current':0,'download_total':0,'upload_current':0,'upload_total':0}
            record_item(uid, source, topic, mid, 'reading', parent=parent)
            active_card = overall
            await _safe_status_edit(active_card, _batch_episode_progress_body(ctx))
            try:
                target = await fetch_message_safely(ub, cache_key, source, mid, active_card)
                if not target or target.empty:
                    state = 'skipped'
                    error = 'Missing/deleted message'
                elif topic and (getattr(target,'reply_to_top_message_id',None) or getattr(target,'message_thread_id',None)
                                or getattr(target,'reply_to_message_id',None)) != topic and target.id != topic:
                    state = 'skipped'
                    ...
                    if has_downloadable_media(target):
                        ok = await retry_flood(transfer_media_original, ub, client, destination, target,
                                               active_card, ctx, user_id=uid, _metadata_deadline=False)
                    else:
                        ctx['non_file'] = True
                        # A network error during a send can leave delivery uncertain.
                        ctx['upload_started'] = True
                        record_item(uid,source,topic,mid,'sending',parent=parent)
                        ok = await retry_flood(send_non_file_content, client, destination, target, user_id=uid)
                    if ok:
                        state = 'sent'
                        consume_usage(uid, 1)
                    else:
                ...
                if state in {'failed', 'uncertain'} and error:
                    reasons = meta.setdefault('failure_details', [])
                    if len(reasons) < 3:
                        reasons.append(f"ID {mid}: {error}")
                processed += 1
                sent += state == 'sent'
                ...
                ctx['finished'] = True
                ctx['ui_seq'] += 1
                ctx['phase'] = ('✅ Episode complete — delivered' if state == 'sent' else
                                f"{'⏭' if state == 'skipped' else '⚠️'} {state.title()} • {error}")
                if state == 'sent':
                    ctx['delivered'] = True
        ...
        rows = []
        if failed:
            rows.append([InlineKeyboardButton("🔁 Retry failed only",callback_data=f"retry:{meta['job_id']}")])
        ...
        if overall:
            await _safe_status_edit(overall, overall_card(kind,len(ids),processed,sent,skipped,failed,uncertain,'Cancelled'))
        raise
    except Exception as exc:
        meta['outcome'] = 'failed'
        await message.reply_text('❌ ' + friendly_error(exc, uid), reply_markup=support_markup())
...
async def handle_new_callback(client, query):
    ...
    if data.startswith('job:'):
        ...
        if action == 'start':
            ...
            gate = JOB_GATES[uid]
            if action == 'resume' and gate.is_set():
                await query.answer('Already running. /status shows the current wait.', show_alert=True)
                return True
...
async def job_control_cmd(client, message: Message):
    ...
    if meta.get('cancel_requested'):
        return await message.reply_text("🛑 Stopping… Please wait for the result.")
    if command == 'resume' and gate.is_set():
        return await message.reply_text("▶ Already running. " + meta.get('phase', 'Processing') + ". /status shows progress; Resume does not restart a transfer.")
    ...
    await message.reply_text(
        ...
        f"Stage: {meta.get('phase', 'Processing')}\n"
        f"Transfer: {_human_bytes(meta.get('transfer_bytes', 0))} / {_human_bytes(meta.get('transfer_total', 0))}\n"
        f"Last byte progress: {max(0, int(time.monotonic() - meta.get('last_progress', time.monotonic())))}s ago\n"
        f"Telegram wait: {max(0, int(meta.get('wait_until', time.monotonic()) - time.monotonic()))}s\n"
        "/pause /resume /cancel\nPause takes effect between messages."
    )

def free_trial_text(uid):
    plan = effective_plan(uid)
    remaining = remaining_quota(uid)
    if is_admin(uid) or plan != 'free':
        return tr(uid, "✅ Your active plan already includes Single Save. Send a post link. /myplan", "✅ உங்கள் plan-ல் Single Save உள்ளது. Post link அனுப்பவும். /myplan")
    left = 'Unlimited' if remaining is None else str(remaining)
    if remaining == 0:
        return tr(uid, "🆓 Today's free saves are finished. Resets at 00:00 UTC (05:30 IST). Use /plans for more saves.", "🆓 இன்றைய free saves முடிந்துவிட்டன. காலை 05:30 IST-க்கு புதுப்பிக்கப்படும். கூடுதல் saves: /plans")
    return tr(uid,
        f"🆓 Free Single Save\nRemaining today: {left}\nNo payment required. Failed downloads do not use a save.\n1. Connect your account with /login.\n2. Paste one Telegram post link.\nBatch/Clone require a paid plan. Resets at 00:00 UTC (05:30 IST).",
        f"🆓 Free Single Save\nஇன்று மீதம்: {left}\nPayment தேவையில்லை. Failed download-க்கு quota குறையாது.\n1. /login மூலம் account இணைக்கவும்.\n2. ஒரு Telegram post link அனுப்பவும்.\nBatch/Clone-க்கு paid plan தேவை. காலை 05:30 IST-க்கு quota புதுப்பிக்கப்படும்.")

@bot.on_message(filters.command("trial") & filters.private)
async def trial_cmd(client, message):
    uid = message.chat.id
    allowed, reason = feature_allowed(uid, 'single', 0)
    await message.reply_text(free_trial_text(uid) if allowed else reason,
                             reply_markup=login_keyboard() if allowed and not has_personal_session(uid) else support_markup())

@bot.on_message(filters.command("version") & filters.private)
async def version_cmd(client, message):
    with open(__file__, 'rb') as source:
        fingerprint = hashlib.sha256(source.read()).hexdigest()[:12]
    await message.reply_text(f"SavePro {BOT_VERSION}\nBuild: {fingerprint}\nQuiet progress • bounded source lookup • safe cancel")
...
async def start_cmd(client, message: Message):
    ...
    with db_conn() as con:
        first_start = con.execute("SELECT 1 FROM users WHERE user_id=?", (user_id,)).fetchone() is None

    if message.from_user and message.from_user.id == message.chat.id:
        touch_user(message)

    pending_login = LOGIN_STATES.pop(user_id, None)
    if pending_login and pending_login.get('client') and pending_login['client'] is not USER_CLIENTS.get(user_id):
        try:
            await bounded_operation(pending_login['client'].disconnect(), 'Closing abandoned login', 5)
        except Exception:
            pass
    ...
    # Persistent Support keyboard only needs to be introduced once.
    if first_start:
        await message.reply_text(
            "🆘 Support is always available from the keyboard below.",
...
async def callback_handler(client, query: CallbackQuery):
    ...
    if query.data == "free_trial":
        await query.answer()
        return await trial_cmd(client, query.message)
    if query.data == "batch_new":
        if user_id in JOB_TASKS:
            return await query.answer("Finish or cancel the current job first.", show_alert=True)
        if user_id in LOGIN_STATES:
            return await query.answer("Finish login or use /cancel first.", show_alert=True)
        allowed, reason = feature_allowed(user_id, "batch", 1)
        if not allowed:
            await query.answer()
            return await query.message.reply_text(reason, reply_markup=upgrade_markup(user_id))
        CLONE_INPUTS.discard(user_id)
        PAYMENT_STATES.pop(user_id, None)
        CUSTOMIZE_STATES.pop(user_id, None)
...
def normalize_login_code(text):
    # Accept digits separated by whitespace; reject pasted notification text.
    if not re.fullmatch(r"[0-9\s]+", text or ''):
        return None
    code = re.sub(r"\s", "", text)
    return code if 5 <= len(code) <= 7 else None

async def discard_login_attempt(uid, state):
    if LOGIN_STATES.get(uid) is state:
        LOGIN_STATES.pop(uid, None)
    temp = state.get('client')
    if temp and USER_CLIENTS.get(uid) is not temp:
        try:
            await bounded_operation(temp.disconnect(), 'Closing login attempt', 5)
        except Exception:
            pass

async def login_failure(client, uid, state, error):
    name = type(error).__name__
    if LOGIN_STATES.get(uid) is not state:
        return
    if name in {'PhoneCodeInvalid', 'PhoneCodeEmpty', 'PasswordHashInvalid'}:
        state['failures'] = state.get('failures', 0) + 1
        if state['failures'] < 5:
            text = ("❌ Incorrect 2FA password. Try again or /cancel." if name == 'PasswordHashInvalid'
                    else "❌ Incorrect code. Enter the latest code for this login request, or /cancel. Your login request is still active.")
            return await client.send_message(uid, text)
    if isinstance(error, FloodWait):
        state['retry_after'] = time.monotonic() + max(1, error.value)
        return await client.send_message(uid, f"⏳ Telegram requires a {max(1, error.value)} second wait. Retry after that; /cancel remains available.")
    await discard_login_attempt(uid, state)
    if name in {'PhoneCodeExpired', 'PhoneCodeHashEmpty', 'PhoneCodeHashInvalid'}:
        text = "⌛ Telegram expired or invalidated this code. This login attempt is closed. Use /login → Phone + OTP to request a fresh code. Old codes cannot be reused."
    elif isinstance(error, asyncio.TimeoutError):
        text = "⌛ Login connection timed out. Use /login again. If Telegram shows a new session, check Settings → Devices first."
    else:
        text = f"❌ Login could not finish ({name}). Use /login again or /support."
    await client.send_message(uid, text, reply_markup=login_keyboard())

async def complete_phone_login(client, uid, state, me, method):
    if LOGIN_STATES.get(uid) is not state:
        await discard_login_attempt(uid, state)
        return
    temp = state['client']
    # Persist only after authentication completed. Never store OTP/password.
    session = await bounded_operation(temp.export_session_string(), 'Saving authenticated session')
    if LOGIN_STATES.get(uid) is not state:
        await discard_login_attempt(uid, state)
        return
    if USER_CLIENTS.get(uid) is not None and USER_CLIENTS[uid] is not temp:
        await bounded_operation(close_user_client(uid), 'Switching authenticated account', 10)
    if LOGIN_STATES.get(uid) is not state:
        await discard_login_attempt(uid, state)
        return
    save_session_string(uid, session)
    USER_CLIENTS[uid] = temp
    LOGIN_STATES.pop(uid, None)
    await client.send_message(uid, "✅ Telegram login successful! Send one post link, or use /trial /batch.")
    await notify_admin_login(me, method)
...
async def login_input_handler(client, message: Message):
    ...
    if state.get('busy'):
        ...
        return await client.send_message(user_id, "⏳ Login request is processing. Please wait, or /cancel.")
    if state.get('retry_after', 0) > time.monotonic():
        await safe_delete_message(message)
        return await client.send_message(user_id, f"⏳ Wait {int(state['retry_after'] - time.monotonic()) + 1} seconds before retrying.")
    ...
    if step == "phone":
        ...
        if not re.fullmatch(r"\+[1-9][0-9]{7,14}", phone):
            state['busy'] = False
            return await client.send_message(user_id, "Send your number with country code, e.g. +91XXXXXXXXXX.")
        state['busy'] = True
        temp = Client(f"phone_login_{user_id}_{secrets.token_hex(4)}", api_id=API_ID,
                      api_hash=API_HASH, in_memory=True, no_updates=True)
        state['client'] = temp
        try:
            await bounded_operation(temp.connect(), 'Connecting login')
            sent = await bounded_operation(temp.send_code(phone), 'Requesting login code')
            if LOGIN_STATES.get(user_id) is not state:
                await discard_login_attempt(user_id, state)
                return
            state.update(step='otp', phone=phone, phone_code_hash=sent.phone_code_hash)
            await client.send_message(user_id, tr(user_id,
                "📩 Check the latest Telegram login code for this request. Enter digits separated by spaces, for example: 1 2 3 4 5. Do not forward the notification. /cancel to stop. This does not restore an expired code.",
                "📩 இந்த login request-க்கு வந்த புதிய code-ஐ பார்க்கவும். இலக்குகளுக்கு இடையில் space வைத்து அனுப்பவும். உதாரணம்: 1 2 3 4 5. Notification-ஐ forward செய்ய வேண்டாம். ரத்து செய்ய /cancel. Expired code இதனால் மீண்டும் செல்லுபடியாகாது."))
        except Exception as error:
            await login_failure(client, user_id, state, error)
        finally:
            state['busy'] = False
        return

    if step in {'otp', 'password'}:
        state['busy'] = True
        await safe_delete_message(message)
        temp = state.get('client')
        if not temp or (step == 'otp' and not state.get('phone_code_hash')):
            await discard_login_attempt(user_id, state)
            return await client.send_message(user_id, "Login request is missing. Use /login again.")
        code = normalize_login_code(text) if step == 'otp' else None
        if step == 'otp' and code is None:
            state['busy'] = False
            return await client.send_message(user_id, "Enter only the 5–7 code digits, optionally separated by spaces. Do not paste notification text.")
        state['busy'] = True
        try:
            if step == 'otp':
                await bounded_operation(temp.sign_in(phone_number=state['phone'],
                    phone_code_hash=state['phone_code_hash'], phone_code=code), 'Verifying login code')
            else:
                await bounded_operation(temp.check_password(message.text or ''), 'Verifying 2FA')
            me = await bounded_operation(temp.get_me(), 'Checking authenticated account')
            await complete_phone_login(client, user_id, state, me, 'Phone + OTP' if step == 'otp' else 'Phone + OTP + 2FA')
        except SessionPasswordNeeded:
            if LOGIN_STATES.get(user_id) is state:
                state.update(step='password', failures=0)
                await client.send_message(user_id, "🔐 Your account requires its 2FA password to finish login. /cancel to stop. Only continue if you trust this bot operator with account access.")
        except Exception as error:
            await login_failure(client, user_id, state, error)
        finally:
            state['busy'] = False
...
async def cancel_cmd(client, message: Message):
    ...
    if state and state.get("client"):
        try:
            await bounded_operation(state["client"].disconnect(), "Closing login", 10)
        except Exception:
            pass
...
def batch_choice_keyboard(user_id=None):
    ceiling = None
    if user_id is not None:
        limits=[v for v in (batch_limit_for_user(user_id),remaining_quota(user_id)) if v is not None and v > 0]
        if remaining_quota(user_id)==0:
            ceiling=0
        elif limits:
            ceiling=min(limits)
        if not is_admin(user_id) and MAX_BATCH_MESSAGES:
            ceiling=min(ceiling,MAX_BATCH_MESSAGES) if ceiling is not None else MAX_BATCH_MESSAGES
    values=[n for n in (5,10,25,50,100,250) if ceiling is None or n<=ceiling]
    if ceiling and ceiling not in values and ceiling<250:
        values.append(ceiling)
    values=sorted(set(values))
    buttons=[InlineKeyboardButton(str(n),callback_data=f'batch_count:{n}') for n in values]
    rows=[buttons[i:i+3] for i in range(0,len(buttons),3)]
    if ceiling!=0:
        rows.append([InlineKeyboardButton('🔢 Custom count',callback_data='batch_custom')])
    if user_id is None or feature_allowed(user_id,'range',1)[0]:
        rows.append([InlineKeyboardButton('🔗 End link',callback_data='batch_end_link')])
    rows.append([InlineKeyboardButton('❌ Cancel',callback_data='batch_wizard_cancel')])
    return InlineKeyboardMarkup(rows)
...
async def handle_batch(client, message: Message):
    ...
    if user_id in JOB_TASKS or ACTIVE_TASKS.get(user_id):
    ...
    await message.reply_text(
        ...
        reply_markup=batch_choice_keyboard(user_id),
    )
...
async def _batch_wizard_text_handler(client, message: Message):
    ...
    if step == "start_link":
        ...
        try:
            parse_telegram_link(links[0])
        except ValueError:
            return await message.reply_text("Send a Telegram post link, not a channel name or invite.")
...
# 13. SINGLE LINK
# ============================================================
@bot.on_message(filters.regex(r"(?i)(?<![\w./])(?:https?://)?t\.me/") & filters.private)
@managed_job("single")
async def handle_single_link(client, message):
    ...
    links=extract_telegram_links(message.text or '')
    if len(links) != 1:
        return await message.reply_text("Send one post link at a time. Use /batch for multiple posts.")
...
async def main():
    ...
    if server_userbot:
        try:
            await bounded_operation(server_userbot.start(), "Starting shared account")
            print("✅ Shared server userbot connected!")
        except Exception as exc:
            print(f"Shared account startup failed: {type(exc).__name__}; personal login remains available")

    await bot.set_bot_commands([
        BotCommand("start", "🏠 Home"),
        BotCommand("trial", "🆓 Free saves / remaining quota"),
        BotCommand("version", "Build version / support"),
        ...
        BotCommand("retry", "🔁 Retry failed items: /retry JOB_ID"),
