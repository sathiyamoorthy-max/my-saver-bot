...
def admin_notification_targets():
    ...
    if ADMIN_USERNAME:
        targets.append(f"@{ADMIN_USERNAME}")
...
async def notify_admins(text: str):
    ...
    for admin_target in targets:
        try:
            await bot.send_message(admin_target, text)
            delivered = True
        except Exception as e:
            print(f"Admin notify failed {admin_target}: {e}")
    return delivered
...
def make_transfer_progress(status_msg, phase: str, msg_id: int):
    state = {"last": 0.0}

    async def progress(current, total):
        now = time.monotonic()
        if current < total and now - state["last"] < TRANSFER_PROGRESS_INTERVAL:
            return
        state["last"] = now
        ...
        try:
            await status_msg.edit_text(
                f"{phase} • ID `{msg_id}`\n"
                f"`{pct:.1f}%` • `{_human_bytes(current)}` / `{_human_bytes(total)}`\n"
                "Stop: `/cancel`"
            )
        except Exception:
            pass
...
async def transfer_media_original(ub, bot_client, dest_chat, target_msg, status_msg=None):
    """Download + upload one media item with live progress, timeout and retry."""
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            download_progress = (
                make_transfer_progress(status_msg, "📥 Downloading", target_msg.id)
                if status_msg else None
            )
            download_coro = ub.download_media(target_msg, progress=download_progress)
            file_path = await asyncio.wait_for(
                download_coro, timeout=MEDIA_DOWNLOAD_TIMEOUT
            )
            if not file_path:
                raise RuntimeError("Telegram returned no downloaded file")

            upload_progress = (
                make_transfer_progress(status_msg, "📤 Uploading", target_msg.id)
                if status_msg else None
            )
            await asyncio.wait_for(
                send_media_original(
                    bot_client, dest_chat, target_msg, file_path, progress=upload_progress
                ),
            ...
            last_error = RuntimeError(
                f"transfer timeout on message {target_msg.id} (attempt {attempt}/{attempts})"
            )
...
async def payment_screenshot_handler(client, message: Message):
    ...
    if not admin_targets:
        await message.reply_text("⚠️ ADMIN_IDS / ADMIN_USERNAME is not configured on the server; payment is saved as pending.")
        return
    ...
    for admin_target in admin_targets:
        try:
            if message.photo:
                await bot.send_photo(admin_target, message.photo.file_id, caption=admin_caption, reply_markup=kb)
            else:
                await bot.send_document(admin_target, message.document.file_id, caption=admin_caption, reply_markup=kb)
            delivered = True
        except Exception as e:
            print(f"Payment proof send to admin {admin_target} failed: {e}")

    if not delivered:
        await message.reply_text(
            "⚠️ Payment is saved as pending, but the admin notification could not be delivered. "
            "Admin should start the bot once and ADMIN_IDS should contain the numeric Telegram user ID."
        )
