...
def settings_keyboard(user_id: int):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✨ Quick Branding Setup", callback_data="brand:start")],
        [
...
async def require_user_client(message: Message):
    ...
    await message.reply_text(
        tr(user_id,"🔐 Connect your Telegram account first. After login, resend your post link or batch request.",
           "🔐 முதலில் Telegram account இணைக்கவும். Login முடிந்ததும் உங்கள் link அல்லது batch request-ஐ மீண்டும் அனுப்பவும்."),
...
async def _safe_status_edit(status_msg, body: str):
    ...
    try:
        ...
        await asyncio.wait_for(status_msg.edit_text(body, reply_markup=job_buttons(uid), parse_mode=ParseMode.DISABLED), timeout=3)
    except Exception:
        pass
...
CONFIRM_SINGLE = os.environ.get("CONFIRM_SINGLE", "false").lower() in {"true", "1", "yes"}
...
async def request_job_confirmation(message, uid, kind, source, topic, ids, destination):
    if not CONFIRM_JOBS or (kind == "single" and not CONFIRM_SINGLE):
        return True
    meta = JOB_META[uid]
    ...
    while True:
        ...
        body = (f"📋 Ready to start • {kind.title()}\n"
                f"Messages: {len(ids)} ID slots • {ids[0]} → {ids[-1]}\n"
                f"Source: {source}" + (f" • Topic {topic}" if topic else "") + "\n"
                f"Delivery: {'this private chat' if destination == uid else str(destination)}\n"
                f"Audio artist: {prefs.get('audio_artist') or 'Original'}\n"
                f"Audio title: {'Custom' if prefs.get('audio_title') else 'Original'}\n"
                f"Audio layout: {'Clean card' if prefs.get('audio_clean_card') else 'With caption'}\n"
                f"Caption: {prefs.get('caption_mode')} • Watermark: {'ON' if prefs.get('watermark_enabled') else 'OFF'}\n"
                "Missing/deleted IDs may be skipped. Start within 5 minutes.\n"
                "Edit branding with Settings, then return here and tap Start.")
        jid = meta['job_id']
        await message.reply_text(body, parse_mode=ParseMode.DISABLED, reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("▶ Start", callback_data=f"job:start:{jid}:{meta['confirm_token']}"),
             InlineKeyboardButton("⚙ Edit branding", callback_data="settings_menu")],
            [InlineKeyboardButton("✏ Edit request", callback_data=f"job:edit:{jid}"),
             InlineKeyboardButton("❌ Cancel", callback_data=f"job:cancel:{jid}")]]))
        try:
...
async def handle_new_callback(client, query):
    ...
    if data == 'help_more':
        await query.answer()
        await query.message.reply_text(tr(uid,
            'Save one: paste a post link. Batch: choose a starting link and count. Clone: start from a selected post. Use /cancel whenever you need to stop.',
            'Single: post link அனுப்பவும். Batch: start link மற்றும் count தேர்வு செய்யவும். Clone: ஒரு post முதல் தொடங்கும். நிறுத்த /cancel.'),
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton('🎧 Branding setup',callback_data='brand:start'),InlineKeyboardButton('🛒 Plans',callback_data='plans_menu')],
                [InlineKeyboardButton('🕘 Job history',callback_data='history_menu'),InlineKeyboardButton('🧾 Payments',callback_data='payments_history')],
                [InlineKeyboardButton('🌐 English / தமிழ்',callback_data='language_menu'),InlineKeyboardButton('📘 Full manual',callback_data='manual_menu')],
                [InlineKeyboardButton('🏠 Home',callback_data='home_menu')]]))
        return True
    if data in {'history_menu','payments_history'}:
        await query.answer()
        await (history_cmd(client,query.message) if data=='history_menu' else payments_cmd(client,query.message))
        return True
    if data == 'home_menu':
        await query.answer()
...
def home_keyboard(uid):
    def label(en, ta): return tr(uid, en, ta)
    rows = [
        [InlineKeyboardButton(label('📥 Save one','📥 ஒரு file'),callback_data='save_single'),
         InlineKeyboardButton(label('📦 Batch','📦 பல files'),callback_data='batch_new'),
         InlineKeyboardButton(label('♻ Clone','♻ Clone'),callback_data='save_clone')],
        [InlineKeyboardButton(label('⚙ Settings','⚙ அமைப்புகள்'),callback_data='settings_menu'),
         InlineKeyboardButton(label('📊 My Plan','📊 என் Plan'),callback_data='my_plan')],
        [InlineKeyboardButton(label('🔐 Login','🔐 Login'),callback_data='login_menu'),
         InlineKeyboardButton(label('❔ Help / More','❔ உதவி / மேலும்'),callback_data='help_more')],
    ]
    if support_row(): rows.append(support_row())
    if is_admin(uid): rows.append([InlineKeyboardButton('🛡 Admin',callback_data='admin_panel')])
    return InlineKeyboardMarkup(rows)

async def close_pending_login(uid):
...
async def start_cmd(client, message: Message):
    ...
    if message.from_user and message.from_user.id==uid:
        touch_user(message)
    for states in (PAYMENT_STATES,BATCH_STATES,CUSTOMIZE_STATES):
        states.pop(uid,None)
    discard_brand(uid)
    CLONE_INPUTS.discard(uid)
    await close_pending_login(uid)
    saved=has_personal_session(uid) or uid in USER_CLIENTS or bool(server_userbot and (ALLOW_SHARED_SESSION or is_admin(uid)))
    instruction=tr(uid,
        'Paste one Telegram post link to save it. For several files, choose Batch or Clone.',
        'ஒரு Telegram post link அனுப்பினால் file வரும். பல files வேண்டுமெனில் Batch அல்லது Clone தேர்வு செய்யவும்.') if saved else tr(uid,
        'First tap Login and connect your Telegram account. Then paste a post link to save it.',
        'முதலில் Login அழுத்தி Telegram account இணைக்கவும். பிறகு post link அனுப்பினால் file வரும்.')
    text='🤖 SavePro\n\n'+instruction+'\n\n'+tr(uid,
        'One progress card. Files arrive in order. One final summary.',
        'ஒரே progress card. Files வரிசையாக வரும். கடைசியில் ஒரு summary மட்டும்.')
    if uid in JOB_TASKS:
        text+='\n\n'+tr(uid,'Your current job is still active. /status','உங்கள் வேலை தொடர்கிறது. /status')
    await message.reply_text(text,reply_markup=home_keyboard(uid))
...
async def callback_handler(client, query: CallbackQuery):
    ...
    if query.data == "batch_new":
        if user_id in JOB_TASKS or user_id in LOGIN_STATES:
            return await query.answer("Finish the current job/login, or /cancel first.", show_alert=True)
    ...
    if step == "otp":
        ...
        try:
            ...
            await client.send_message(
                ...
                + tr(user_id,"\n\nReady! Paste one post link to save. For several files, choose Batch.","\n\nதயார்! ஒரு post link அனுப்பவும். பல files-க்கு Batch தேர்வு செய்யவும்."),
...
async def handle_batch(client, message: Message):
    ...
    if user_id in JOB_TASKS:
        return await message.reply_text("⚠️ A job is active. Use /status or /cancel.")
...
async def _batch_wizard_text_handler(client, message: Message):
    ...
    if step == "start_link":
        ...
        try:
            ...
            return await message.reply_text(tr(user_id,"Send a post link, not just a channel name/invite.","Channel பெயர்/invite மட்டும் இல்லாமல் ஒரு post link அனுப்பவும்."))
...
# 13. SINGLE LINK
# ============================================================
@bot.on_message(filters.regex(r"(?i)(?:https?://)?t\.me/") & filters.private)
@managed_job("single")
async def handle_single_link(client, message):
    ...
    links=extract_telegram_links(message.text or '')
    if len(links) > 1:
        return await message.reply_text(tr(uid,'Send one link for Single Save. For a range, use /batch with the start and end links.','Single Save-க்கு ஒரு link அனுப்பவும். பல files-க்கு /batch பயன்படுத்தவும்.'))
    if links:
        await run_transfer_job(client,message,uid,'single',start_link=links[0])

@bot.on_message(filters.private & filters.text, group=3)
async def plain_text_help(client,message):
    uid=message.chat.id
    text=(message.text or '').strip()
    if text.startswith('/') or extract_telegram_links(text) or uid in JOB_TASKS or uid in CLONE_INPUTS:
        return
    if any(uid in state for state in (LOGIN_STATES,PAYMENT_STATES,BATCH_STATES,CUSTOMIZE_STATES,BRAND_STATES)):
        return
    if text.lower() in {'support','🆘 support','🆘 support / contact admin'}:
        return
    await message.reply_text(tr(uid,'Send a Telegram post link, or choose an action below.',
                                'Telegram post link அனுப்பவும் அல்லது கீழே ஒரு option தேர்வு செய்யவும்.'),
