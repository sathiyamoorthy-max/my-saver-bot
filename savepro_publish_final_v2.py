# SavePro FINAL RELEASE CANDIDATE — 2026-09-23
...
PEER_SYNC_TIMEOUT = max(30, int(os.environ.get("PEER_SYNC_TIMEOUT", "90")))
...
async def fetch_message_safely(ub_client, cache_key, chat_id, msg_id, status_msg=None):
    try:
        ...
        print(f"⚠️ Peer lookup retry [{cache_key}] chat={chat_id}: {type(first_error).__name__}")

        # A full dialogs sync can be expensive. Do it once when needed, then
        # at most once every 5 minutes for the same connected account.
        now = time.monotonic()
        ...
        if status_msg:
            try:
                await _safe_status_edit(status_msg, "🔄 Refreshing Telegram access…")
            except Exception:
                pass
        ...
        try:
            ...
            raise RuntimeError("Telegram chat refresh timed out. Try again.") from sync_error
    ...
    async def progress(current, total):
        ...
        async def do_edit():
            try:
                ...
                # "file complete" or final summary card.
        ...
        # UI editing runs in the background. File download/upload must not wait
        # for Telegram to edit a progress message.
